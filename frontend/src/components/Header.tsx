...
export const HeaderPart: React.FC<HeaderPartProps> = ({ cart = [] }) => {
  const [isCartOpen, setIsCartOpen] = React.useState<boolean>(false);
  const [isScrolled, setIsScrolled] = React.useState<boolean>(false);
            ...
            <ListItem key={index} className="flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {item.quantity} x {item.amount.toLocaleString()}₮
                </p>
              </div>
  ...
  return (
    <header
      className={`fixed w-full top-0 flex  justify-center itmes-center z-50 bg-white transition-all duration-300 ${
        isScrolled ? "shadow-md" : "border-b border-gray-100"
      }`}
    >
      <div className="container mx-auto px-4 py-4 ">

...
//               <button
//                 onClick={() =>
//                   alert("Таны бараа амжилттай сагсанд нэмэгдлээ! 🎉")
//                 }
...
export const FoodModal: React.FC<FoodModalProps> = ({
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = Math.max(1, parseInt(e.target.value, 10)); // Prevent 0 or negative numbers
    setQuantity(value);
  };

  const handleAdd = () => {
    onAddToCart({ img, text, amount, quantity });
    onClose();
  };
